<template>
    <div>
        <div
            ...
            <img id="all_pick" style="height: 20px;margin: 5px 0px 5px 12px;" v-if="all_pick_flag"
                src="../../assets/img/choice2.png" @click="all_pick" />
            <img id="all_pick" style="height: 20px;margin: 5px 0px 5px 12px;" v-else src="../../assets/img/choice1.png"
                @click="all_pick" />
            <div style="font-size: 14px;margin: 5px 0px 5px 12px;">全选</div>
            <div style="font-size: 14px;margin: 5px 0px 5px 0px;color: rgb(48, 152, 251)">
                （{{callPhoneList.length}}/{{callPhoneList_length}}）</div>
        </div>
        <van-list :id="departId" v-model="loading" :finished="finished" @load="onLoad" :offset="60" :error.sync="error"
