    ...
    >
      ...
      >
        <input
          ...
          :id="item.dingid+index"
          hidden
          type="checkbox"
          :value="item.dingid"
          v-model="callPhoneList"
          @change="addPhone($event)"
        />
        <label @click="errorMsg(item)" v-if="item.realname != '刘锋' && item.realname != '张志川'" :for="item.dingid+index" class="active"></label>
        ...
        <div style="display: flex; position: absolute; right: 10px;top: 20px;">
           <img
            v-if="item.realname != '刘锋' && item.realname != '张志川' "
