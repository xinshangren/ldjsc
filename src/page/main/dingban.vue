  ...
  <div>
    <van-tabs ref="tabId1" id="tabId1" v-model="active" :sticky="false" :ellipsis="false" line-width="20%"
      ...
      <van-tab v-for="(item,index)  in tabList" :key="index" :title="item.departname">
        <child1 v-on:addPhone="addPhone" :callPhoneList_p="callPhoneList" :departId="item.id" :depart_flag="index">
        </child1>
      </van-tab>
    ...
    <div id="callButton" style="z-index: 3;display: none; width: 100%; height: 53px; position: absolute;bottom: 0px;
      ...
      <div style=" width:100%;vertical-align: middle;display: flex;margin: 18px 0px 0px 0px;">
        ...
        <div style="font-size: 14px;width: 49%; margin-left: 1px;color: rgb(48, 152, 251) ">{{callPhoneList.length}}人
        </div>
  ...
  export default {
    ...
    methods: {
        ...
        if (map != null) {
          ...
          if (this.child_phoneList.length > 0) {
            this.child_phoneList.forEach(element => {
              ...
              if (map.del_list != null && map.del_list.length > 0) {
                console.log(map.del_list)
                console.log(element);
                map.del_list.forEach(e => {
                  if (element.callPhoneList.indexOf(e) > -1) {
                    var index = element.callPhoneList.indexOf(e);
                    element.callPhoneList.splice(index, 1);
                  }
                });
              }
            });
      ...
      changeactive: function (data) {
        this.$root.$emit("test11", this.callPhoneList);
      },
