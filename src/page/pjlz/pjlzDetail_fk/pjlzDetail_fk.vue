  ...
  <div style="margin-top:54px;background:#f7f7f7">
    ...
    <div style="margin:10px 0px 0px 0px;background:#ffffff;min-height: 250px;">
      <div style="padding:10px 15px 10px 15px;position: relative;">
        ...
        <textarea v-model="fk_content" placeholder="请输入反馈内容"
        style="font-size: 15px;margin-top: 10px;border: 1px solid #9f9f9f;border-radius: 10px;resize: none;width: 100%;height: 70px;">
      </textarea>
        <van-uploader v-model="file_list" multiple :after-read="afterRead" :preview-image="false" accept="*">
        ...
        <div v-for="(item,index) of file_list" style="display: flex;font-size: 14px;color:#2599e6 ;">
          ...
          <img @click="delete_adjunct(index)" style="height: 20px;margin-left: 5px;"
        ...
        <div style="display: flex;background: #28bcfe;border-radius:20px;width: 64%;
                ...
                margin-top: 15px;
                height: 40px;">
          <div style="margin: auto;margin-top: 6px;margin-bottom: 6px; color: #ffffff;font-size: 18px;">提交</div>
        </div>
      </div>
    </div>
  </div>
        ...
        httpMethod
          ...
          .then(res => {
            ...
            if (res.success == "1") {
              ...
              if (self.pj_detail.approval_manage_person != null && self.pj_detail.approval_manage_person != "") {
