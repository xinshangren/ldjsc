<template style="background: #ffffff;">
  <div>
    <div @click="closeSelect($event)" id="gkmaintitleid" class="gkmaintitle">
      <div
        id="gkdatepick"
      ...
      <div
        style="margin-left: 15%;line-height: 70px;vertical-align: middle;font-size: 14px;color:#ff3d2e;"
      >评级点位：</div>
      <div
        style="line-height: 70px;vertical-align: middle;font-size: 26px;color:#ff3d2e;"
      >{{datatj.dw_zs}}</div>
    ...
    <div
      id="gkdateselect"
      ref="gkdateselect"
      style="display: none; background:#ffffff;position: absolute;width:70px;z-index: 998;left:10%;top:107px;border:0.5px #e03e3c solid;"
    >
      <ul id="gkdateul">
        <li
          show="6月份"
    ...
    <!-- <div style="position:absolute;">11</div> -->
    ...
    <div style="margin-left: 20px;margin-right: 20px;border: 1px #F1F1F1 solid;">
      ...
      <van-grid :border="true" :column-num="7">
        <van-grid-item v-for="(value,index) in MonthLevelist" :key="index">{{value}}</van-grid-item>
      </van-grid>
    </div>
...
export default {
  ...
  mounted() {
    ...
    $("#gkdatepick").on("click", function() {
      var isShow = $("#gkdateselect").css("display");
      if (isShow == "block") {
  ...
  methods: {
    ...
    getOneEchars: function(data) {
      echarsEnti.createEcharsOne(
        echarts,
        this.$refs.myCharts1,
        data,
        this.$refs.gkdateselect
      );
    },
    //初始化第一个图表
