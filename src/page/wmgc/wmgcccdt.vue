        ...
        <li v-for="(item,index) in list" :key="index"
          ...
          <div style="position: relative;">
            ...
            <div style="margin-left: 10px;display: flex;line-height: 13px;">
              <div style="font-size: 10px;color: #333333;white-space: nowrap;overflow: hidden;text-overflow:ellipsis;">{{item.title}}</div>
            </div>
...
<script>
  ...
  Vue.use(PullRefresh).use(Swipe).use(SwipeItem);
          ...
          .then(res => {
            if (res.success == "1") {
              ...
              for (var i = 0; i < res.dataList.length; i++) {
                _this.images.push(httpMethod.returnBaseUrlFun()+res.dataList[i].imgUrl);
